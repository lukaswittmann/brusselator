...
@ray.remote
def draw_figure(i,h, print, n):
    plt.clf()
    plt.imshow(print[:, :], cmap='RdBu', interpolation="lanczos")  # , cmap='hot', interpolation="nearest", cmap='hsv', interpolation="lanczos" bilinear
    plt.axis('off')
    plt.clim(0.2, 3.5)
    plt.title("t=" + str(np.round((dt * i), decimals=1)) + ", Iteration " + str(i),fontsize=8)
    plt.tight_layout()
    plt.savefig("render/image_" + str(h) + ".png", dpi=350)
